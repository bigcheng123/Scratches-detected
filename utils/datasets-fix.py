...
class LoadStreams:  ##### main.py 中 dataset = LoadStreams(self.source, img_size=imgsz, stride=stride)
    ...
    def update(self, i, cap, stream):
        ...
        while cap.isOpened() and n < f:
            ...
            if not streams_update_flag:
                cap.release()

    def release_camera(self, thread_list):
        print('Excute funtion release camera------------------------------', len(thread_list))
        # thread_list = self.threads
        for thread in thread_list:
            logging.info("Excute stop thread ...")
            # thread.stop()
    ...
    def __next__(self):
        ...
        if not all(x.is_alive() for x in self.threads) or cv2.waitKey(1) == ord(
            ...
            # for thread in self.threads: # TODO: BUG
            #     if thread.is_alive():
            #         print('thread type', type(thread)) # <class 'threading.Thread'>
            #
